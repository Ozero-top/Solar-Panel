...
function applyRoleUI() {
  if (state.role === 'viewer') {
    ...
    // 账号页：禁用修改密码（显示名称仍可修改）
    document.querySelectorAll('#pwdForm input, #pwdForm button').forEach(el => { el.disabled = true; });
...
const SETTING_SECTIONS = {
  basic:    { label: '基础信息', keys: ['site_title', 'site_url', 'footer', 'default_lan_mode', 'home_view', 'announcement_show', 'announcement'] },
  theme:    { label: '外观布局', keys: ['default_theme', 'theme_style', 'content_maxwidth', 'content_pad_lr', 'content_pad_top', 'content_pad_bottom', 'card_style', 'search_width'] },
  media:    { label: 'Logo 与壁纸', keys: ['site_logo', 'wallpaper', 'mask_opacity', 'wallpaper_blur'] },
  clock:    { label: '时钟与天气', keys: ['clock_show', 'weather_show', 'weather_city'] },
