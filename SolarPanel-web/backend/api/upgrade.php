...
if ($action === 'apply') {
    ...
    foreach ($ordered as $rel) {
        ...
        if (!$ok) {
            $why = @is_writable($td)
                ...
                : 'Web 运行进程对目录 ' . $td . ' 无写权限（属主不是站点运行用户）——'
                . '请在面板将 frontend/assets 及其子目录权限设为 755、属主设为站点运行用户（如 www），改一次后本程序即可自行升级';
    ...
    if ($failed) {
        // 保留 staging 与 backup 便于重试 / 手动回滚；code=2 让前端走失败分支展示明细
        json_out(2, [
            'failed' => $failed,
            'updated' => $updated,
            'deleted' => $deletedCount,
            'backup' => 'frontend/uploads/upgrade_backup/' . $token,
        ], '部分文件升级失败，站点可能处于混合版本状态：请重试应用升级，或下载完整包手动覆盖。'
            . '被替换 / 删除的原文件已备份于 frontend/uploads/upgrade_backup/' . $token);
    }
