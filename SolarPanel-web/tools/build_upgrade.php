...
if ($oldArg === '' || in_array('-h', $argv, true) || in_array('--help', $argv, true)) {
    fwrite(STDERR, "用法: php -d phar.readonly=0 {$SELF} <旧版完整包.zip 或 旧版目录> [新版目录=项目根] [输出.zip]\n");
    exit(1);
}
...
if ($from === null || $to === null) {
    fwrite(STDERR, "错误：无法从两侧 api.js 提取 APP_VERSION（from=" . var_export($from, true) . " to=" . var_export($to, true) . "），请人工检查\n");
    exit(1);
}
...
/* ---- 输出路径 ---- */
$out = $outArg !== '' ? $outArg : dirname($newBase) . '/SolarPanel-upgrade-' . $from . '-to-' . $to . '.zip';
$outDir = dirname($out);
if (!is_dir($outDir)) {
